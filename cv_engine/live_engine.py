"""Live browser/server-camera tracking engine."""
...
class LiveEngine:
    ...
    def start(self):
        ...
        # IMPORTANT:
        # Do NOT use camera.start() on Render.
        # The browser owns the webcam.
        if not self.camera.start_browser():
            raise RuntimeError(
                self.camera.error
                or 'Unable to start browser camera'
            )
    ...
    def _loop(self):
        ...
        with self.app.app_context():
            ...
            while self._running:
                ...
                # Browser may not have uploaded first frame yet.
                if frame is None:
                ...
                if dets:
                    ...
                    payload = {
                        ...
                        'angular_velocity_h_deg_s':
                            round(
                                av_h,
                                3
                            ),

                        'angular_velocity_v_deg_s':
                            round(
                                av_v,
                                3
                            ),
                    ...
                    if (
                        locked
                        and not self._was_locked
                    ):

                        self._alert(
                            'SUCCESS',
                            (
                                'Live target lock acquired '
                                f'· error {te:.2f}°'
                            )
                        )
